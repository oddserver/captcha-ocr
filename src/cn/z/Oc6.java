...
public class Oc6 {
	...
	public static String doujia(int k)throws Exception{
		...
		final Map<BufferedImage, String> mapt = CommonUtil.loadTrainData("/svm/Ocr6");
		String result = useSvm ? "svm_" : "";
	...
	public static BufferedImage doujiaRm2(BufferedImage img) throws Exception{
		Map co=new HashMap<String, String>();
		Map<Object,ArrayList<HashMap<String, String>>> map = new HashMap<Object,ArrayList<HashMap<String, String>>>();
		...
		List<Map.Entry<String, Integer>> vccodEntries=infoIds.subList(0,1);
		Map set=new HashMap<String, String>();
		for(Entry<String, Integer> v:vccodEntries){
			set.put(v.getKey(), v.getValue());
		}
	...
	public static void main(String[] args) throws Exception {
		// ---step1 downloadImage
		String url = "http://www.woying.com/inc/code_phone_send_valid.aspx?1463640292550";
		// 下载图片
		int num=1000;
		CommonUtil.downloadImage(url, clazz,num);
		...
		final Map<BufferedImage, String> mapt = CommonUtil.loadTrainData("/svm/Ocr6");
		 ...
		 for (int i = 0; i < num; ++i) {
		 BufferedImage img = ImageIO.read(new File("img/" + clazz + "/"+i+".png"));;
		 List<BufferedImage> listImg = subimg(img);
		int ii=0;
		String result = useSvm ? "svm_" : "";
		for(BufferedImage list:listImg){
			doujiaRm2(list);
			BufferedImage r = splitImage(list).get(0);
			CommonUtil.deal2img(img);
			CommonUtil.deal2img(img);
			BufferedImage imgdest = new BufferedImage(10, 10, list.getType());
			imgdest = sf.filter(r, imgdest);
			result+=getSingleCharOcr(imgdest,mapt,ii);
			//ImageIO.write(imgdest, "PNG", new File("result/" + clazz + "/" + i+"_"+ii +".png"));
			ii++;
		}
		System.out.println(i+".png="+result);
		ImageIO.write(img, "PNG", new File("result/" + clazz + "/" + i+"_"+result+".png"));
		 }
		

	}

	private static List<BufferedImage> subimg(BufferedImage img) throws Exception {
		List<BufferedImage> subImgs = new ArrayList<BufferedImage>();
		subImgs.add(img.getSubimage(0, 0, 17, 30));
		subImgs.add(img.getSubimage(17, 0, 17, 30));
		subImgs.add(img.getSubimage(34, 0, 17, 30));
		subImgs.add(img.getSubimage(51, 0, img.getWidth()-17*3, 30));
		return subImgs;
	}
