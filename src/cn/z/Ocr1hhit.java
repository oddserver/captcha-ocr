...
public class Ocr1hhit {
	...
	public static String getAllOcr(String file,Map<String, Integer> mapc,int k) throws Exception {
		...
		for(BufferedImage list:listImg){
			...
			//ImageIO.write(list, "png", new File("result/" + clazz + "/" + k+"_"+i +".png"));
			i++;
		}
	...
	public static void main(String[] args) throws Exception {
		// ---step1 downloadImage
		String url = "http://zfxk.hhit.edu.cn/CheckCode.aspx";
		// 下载图片
		int num=200;
		...
		for (int i = 0; i < num; ++i) {
			final String text = getAllOcr("img/" + clazz + "/" + i + ".gif",map,i);
		...
		/*final BufferedImage img = ImageIO.read(new File("D:/Dev_Doc/Project/Java/app/Captcha/result/Ocr1test/0_nrruu.png"));
		for(int i=0;i<img.getHeight();i++){
			for(int j=0;j<img.getWidth();j++){
				if(isWhite(img.getRGB(j, i), 1)==0){
					System.out.print(1);
				}else{
					System.out.print(0);
				}
		...
		//deal2img(img);
		ImageIO.write(img, "PNG", new File("result/" + clazz + "/" +1+"_"+11+ ".png"));*/
	}
